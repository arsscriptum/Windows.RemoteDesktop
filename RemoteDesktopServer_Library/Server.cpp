...
void RemoteDesktop::Server::_Handle_ConnectionRequest(Packet_Header* header, const char* data, std::shared_ptr<RemoteDesktop::SocketHandler>& sh){
	...
	if (uname.size() > 1){
		...
		LOG_TRACE("SERVER", "Connect Request from %s. Computer: \"%s\".", ws2s(uname).c_str(), ws2s(sh->Connection_Info.computername).c_str());
		LOG_TRACE("SERVER", "Connect Request Domain %s. NAME: \"%s\".. IP: \"%s\".", ws2s(sh->Connection_Info.domain).c_str(), ws2s(sh->Connection_Info.full_name).c_str(), ws2s(sh->Connection_Info.ip_addr).c_str());
		...
		LOG_ERROR("SERVER", "Setting user to disconnected because there was no name sent with the access request.");
